...
class GFNModule(ABC, nn.Module):
    ...
    def forward(self, states: States, entropy_coefficient: float) -> TT["batch_shape", "output_dim", float]:
        ## concatenate the states and entropy_coefficient

        ## convert float to (batch_shape, 1) tensor

        states = self.preprocessor(states).to(self.device)
        entropy_coefficient = entropy_coefficient.unsqueeze(0).repeat(states.shape[0], 1).to(self.device)
        
        states = torch.cat((states, entropy_coefficient), dim=1)

        out = self.module(states)
